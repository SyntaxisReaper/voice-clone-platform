...
export default function CyberHelpPage() {
  ...
  // Toaster is rendered at the bottom of the page; use toast.success/toast.error
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    if (!reportForm.description.trim()) {
      toast.error('Please provide a description of the security issue.', { id: 'desc-missing' })
      return
    }
    
    if (!reportForm.anonymous && !reportForm.email.trim()) {
      toast.error('Please provide an email address or submit anonymously.', { id: 'email-missing' })
      return
    }
    
    if (reportForm.description.trim().length < 10) {
      toast.error('Description must be at least 10 characters long.', { id: 'desc-too-short' })
      return
    }
    ...
    try {
      ...
      toast.success('Report submitted. Thank you for helping us improve security.', { id: 'report-ok', duration: 7000 })
      ...
      toast.error(`Submission failed: ${errorMessage}`, { id: 'report-failed', duration: 10000 })
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'threats' && (
          <div className="glass-card p-6">
            <h3 className="text-lg font-semibold text-navy mb-2 flex items-center space-x-2">
              <AlertTriangle className="w-5 h-5 text-orange-600" />
              <span>Threat alerts view coming soon</span>
            </h3>
            <p className="text-navy/70">This section will show live alerts and prevention tips.</p>
          </div>
        )}
