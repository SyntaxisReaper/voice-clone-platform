...
export class AdvancedTTSEngine {
  ...
  async generateAdvancedSpeech(
    ...
    try {
      ...
      throw new Error(`Advanced TTS generation failed: ${error.message}`);
    }
  }
  ...
  async generateMorphedVoice(
    ...
    try {
      ...
      throw new Error(`Voice morphing failed: ${error.message}`);
    }
  }
...
class BatchJob {
    ...
    try {
      for (let i = 0; i < this.request.texts.length; i++) {
        ...
        try {
          ...
          this.result.errors.push(`Item ${i + 1}: ${error.message}`);
        }
      ...
      this.result.errors.push(`Batch processing failed: ${error.message}`);
    } finally {
      this.updateProgress();
    }
  }
  ...
  private async concatenateOutputs(): Promise<void> {
    ...
    try {
      ...
      this.result.errors.push(`Concatenation failed: ${error.message}`);
    }
  }
