...
export class VoiceTrainingPipeline {
  ...
  async startTraining(
    ...
    job.start().catch(error => {
      ...
      this.onTrainingComplete(voiceId, false, error.message);
    });
  ...
  async assessVoiceQuality(voiceId: string, sampleAudio?: ArrayBuffer): Promise<QualityAssessment> {
    try {
      ...
      if (sampleAudio) {
        analytics = await voiceCloningEngine.analyzeVoice(sampleAudio);
      } else {
        ...
        analytics = await voiceCloningEngine.analyzeVoice(arrayBuffer);
      }
...
class TrainingJob {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.updateProgress('completion', 0, `Training failed: ${error.message}`);
      throw error;
    } finally {
